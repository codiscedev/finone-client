...
interface RecurringBillAddDrawerProps {
  isOpen: boolean;
  onClose: () => void;
  onAdd: (bill: {
    name: string;
...
const CATEGORIES = ["Shopping", "Groceries", "Entertainment", "Travel", "Rent", "Transportation"];
const PAYMENT_METHODS = ["UPI", "Net Banking", "Credit Card", "Debit Card", "Cash", "Cheque"];

export default function RecurringBillAddDrawer({
  isOpen,
  onClose,
  onAdd,
}: RecurringBillAddDrawerProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.name.trim() || !form.dateOfDebit || !form.amount || !form.frequency || !form.category || !form.paymentMethod) return;

    onAdd({
      name: form.name,
      dateOfDebit: form.dateOfDebit,
    ...
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative flex flex-col h-screen w-full max-w-[540px] bg-white border-l border-zinc-200 shadow-2xl z-10 transition-transform duration-300 transform translate-x-0 animate-in slide-in-from-right overflow-hidden">
        ...
        <div className="flex h-16 shrink-0 items-center justify-between px-6 border-b border-zinc-150/70 bg-zinc-50/50">
          <div>
            <h3 className="text-sm font-black text-zinc-900 leading-none">Add Recurring Bill</h3>
          ...
          <div className="space-y-5">
            ...
            <FormField label="Day of Debit" required>
              <input
                ...
                className={inputCls}
                required
                value={form.dateOfDebit}
                onChange={(e) => setForm((p) => ({ ...p, dateOfDebit: e.target.value }))}
                placeholder="e.g. 5"
              />
            </FormField>
